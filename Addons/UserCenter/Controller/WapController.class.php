...
class WapController extends AddonsController {
	...
	function borrowRecord(){
		...
		if($info != null){
			//搜索借书记录
			$data['apply_id'] = $info['uid'];
			$data['state'] = 1;
			$list = M('borrow_apply')->where($data)->order('id desc')->select();
			if($list != null){
				foreach ($list as $k => $v){
					//echo "你好";
					$book = M('books')->where('id = '.$list[$k]['book_id'])->find();
					$user = D ( 'Common/User' )->getUserInfo($book['ownner']);
					
					$list[$k]['book_name'] = $book['name'];
					$list[$k]['writer'] = $book['writer'];
					$list[$k]['ownner'] = $user['nickname'];
		...
		if($info != null){
			...
			if($list != null){
				foreach ($list as $k => $v){
					//echo "你好";
					$book = M('books')->where('id = '.$list[$k]['book_id'])->find();
					$user = D ( 'Common/User' )->getUserInfo($list[$k]['apply_id']);
					
					$list[$k]['book_name'] = $book['name'];
					$list[$k]['writer'] = $book['writer'];
					$list[$k]['apply_id'] = $user['nickname'];
					$list[$k]['create_time'] = date("Y-m-d h:i:s",$list[$k]['create_time']);
				}
				$this->assign("info",$info);
				$this->assign("list",$list);
	...
	//荐书记录：
	function adviseRecord(){
		$info = get_followinfo( $this->mid );
		if($info != null){
			//搜索借出记录
			//$data['ownner_id'] = $info['uid'];
			$list = M('books')->where("ownner = ".$info['uid'])->order('id desc')->select();
			if($list != null){
				foreach ($list as $k => $v){
					//echo "你好";
					//$book = M('books')->where('id = '.$list[$k]['book_id'])->find();
