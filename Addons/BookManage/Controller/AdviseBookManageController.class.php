...
class AdviseBookManageController extends AddonsController{
	...
	public function bookList(){
		...
		$this->display (ONETHINK_ADDON_PATH . 'BookManage/View/default/AdviseBookManage/booklist.html');
	}
	//删除书本
	function delbook(){
		$bookid = $_GET['id'];
		...
		if(M('advise_books')->where('id = '.$bookid)->delete()){
			return true;
		}
		else{
			return false;
	...
	function delByChoose(){
		...
		if($ids ==null){
			...
			if(M('advise_books')->where($where)->delete()){
				$this->success("已经删除！");
			}
			else{
				$this->error('删除失败！');
	...
	function editbook(){
		$id = $_GET['id'];
		$book = M("advise_books")->where("id=".$id)->limit(1)->find();
		if($book == null){
			$this->error ( '数据不存在！' );
		}
		else{
	...
	function updateBook(){
		$id = $_GET['id'];
		$kind = $_POST['kind'];
		if(M("advise_books")->where("id=".$id)->limit(1)->find()){
			$book['id'] = $id;
	...
	function agree(){
		...
		if($ids ==null){
			...
			$where = array('id'=>array('in',$ids));
			$books = M('advise_books')->where($where)->field('name,writer,description,ownner,kind,create_time,available')->select();
			if($books != null){
				/*foreach($books as $k => $v){
					$data = $books[$k];
					
					$mark = M('books')->add($data); 	
				}
				if($mark){
						M('advise_books')->where($where)->delete();
						$this->success("已经通过审核");
				}
				else{
					$this->error('审核失败！');
				}*/
				if(M('books')->addAll($books)){
					M('advise_books')->where($where)->delete();
					$this->success("已经通过审核");
